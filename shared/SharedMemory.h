...
class SharedMemory
{
	...
	private:
		...
		void CreateSyncObjects(const tr1::shared_ptr<SECURITY_ATTRIBUTES>& sa, SyncObjectTypes syncObjects, const wstring& strName);
	...
	private:
		...
		tr1::shared_ptr<void>	m_hSharedMem;
		tr1::shared_ptr<T>		m_pSharedMem;

		tr1::shared_ptr<void>	m_hSharedMutex;
		tr1::shared_ptr<void>	m_hSharedReqEvent;
		tr1::shared_ptr<void>	m_hSharedRespEvent;
};
...
void SharedMemory<T>::Create(const wstring& strName, DWORD dwSize, SyncObjectTypes syncObjects, const wstring& strUser)
{
	...
	tr1::shared_ptr<SECURITY_ATTRIBUTES>	sa;
	EXPLICIT_ACCESS					ea[2];

	SID_IDENTIFIER_AUTHORITY	SIDAuthCreator	= SECURITY_CREATOR_SID_AUTHORITY;

	PSID						tmpSID = NULL;
	tr1::shared_ptr<void>			creatorSID;		// PSID

	PACL						tmpACL = NULL;
	tr1::shared_ptr<ACL>				acl;
	...
	m_hSharedMem = tr1::shared_ptr<void>(::CreateFileMapping(
	...
	m_pSharedMem = tr1::shared_ptr<T>(static_cast<T*>(::MapViewOfFile(
													m_hSharedMem.get(), 
													FILE_MAP_ALL_ACCESS, 
													0, 
													0, 
...
void SharedMemory<T>::Open(const wstring& strName, SyncObjectTypes syncObjects)
{
	...
	m_hSharedMem = tr1::shared_ptr<void>(::OpenFileMapping(
										FILE_MAP_ALL_ACCESS, 
										FALSE, 
										m_strName.c_str()),
									::CloseHandle);
	...
	if (syncObjects > syncObjNone) CreateSyncObjects(tr1::shared_ptr<SECURITY_ATTRIBUTES>(), syncObjects, strName);
...
template<typename T>
void SharedMemory<T>::CreateSyncObjects(const tr1::shared_ptr<SECURITY_ATTRIBUTES>& sa, SyncObjectTypes syncObjects, const wstring& strName)
{
	if (syncObjects >= syncObjRequest)
	{
		m_hSharedMutex = tr1::shared_ptr<void>(
		...
		m_hSharedReqEvent = tr1::shared_ptr<void>(
	...
	{
		m_hSharedRespEvent = tr1::shared_ptr<void>(
