...
void ConsoleHandler::ReadConsoleBuffer()
{
	...
	tr1::shared_ptr<void> hStdOut(::CreateFile(
								L"CONOUT$",
...
//	ReadConsoleBuffer();
	...
	tr1::shared_ptr<void> parentProcessWatchdog(::OpenMutex(SYNCHRONIZE, FALSE, (LPCTSTR)((SharedMemNames::formatWatchdog % m_consoleParams->dwParentProcessId).str().c_str())), ::CloseHandle);
							...
							m_consoleParams->dwRefreshInterval)) != WAIT_OBJECT_0)
	{
		...
		switch (dwWaitRes)
		{
			...
			case WAIT_OBJECT_0 + 2 :
			{
				...
				tr1::shared_ptr<wchar_t>	textBuffer;
