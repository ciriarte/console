...
tr1::shared_ptr<BackgroundImage> ImageHandler::GetImage(const ImageData& imageData)
{
	tr1::shared_ptr<BackgroundImage>	bkImage(new BackgroundImage(imageData));
	...
	if (!LoadImage(bkImage)) return tr1::shared_ptr<BackgroundImage>();
...
tr1::shared_ptr<BackgroundImage> ImageHandler::GetDesktopImage(ImageData& imageData)
{
	if (!GetDesktopImageData(imageData)) return tr1::shared_ptr<BackgroundImage>();
...
void ImageHandler::UpdateImageBitmap(const CDC& dc, const CRect& clientRect, tr1::shared_ptr<BackgroundImage>& bkImage)
{
...
bool ImageHandler::GetDesktopImageData(ImageData& imageData)
{
	...
	if (keyColors.QueryStringValue(L"Background", strBackground.GetBuffer(dwDataSize), &dwDataSize) != ERROR_SUCCESS)
	{
		...
		return tr1::shared_ptr<BackgroundImage>();
	}
	strBackground.ReleaseBuffer();
...
bool ImageHandler::LoadImage(tr1::shared_ptr<BackgroundImage>& bkImage)
{
	USES_CONVERSION;
...
void ImageHandler::CreateRelativeImage(const CDC& dc, tr1::shared_ptr<BackgroundImage>& bkImage)
{
	CriticalSectionLock	lock(bkImage->updateCritSec);
...
void ImageHandler::CreateImage(const CDC& dc, const CRect& clientRect, tr1::shared_ptr<BackgroundImage>& bkImage)
{
	CriticalSectionLock	lock(bkImage->updateCritSec);
...
void ImageHandler::PaintTemplateImage(const CDC& dcTemplate, int nOffsetX, int nOffsetY, DWORD dwSrcWidth, DWORD dwSrcHeight, DWORD dwDstWidth, DWORD dwDstHeight, tr1::shared_ptr<BackgroundImage>& bkImage)
{
...
void ImageHandler::TileTemplateImage(const CDC& dcTemplate, int nOffsetX, int nOffsetY, tr1::shared_ptr<BackgroundImage>& bkImage)
{
...
void ImageHandler::TintImage(const CDC& dc, tr1::shared_ptr<BackgroundImage>& bkImage)
{
	CDC				dcTint;
	CBitmap			bmpTint;
